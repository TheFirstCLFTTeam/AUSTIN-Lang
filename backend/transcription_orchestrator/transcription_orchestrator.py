...
async def transcribe(file: UploadFile = File(...)):
    ...
    if skip_transcription:
        ...
        transcription_server_host = os.getenv(
            "TRANSCRIPTION_SERVER_HOST", "transcription-server"
        )
        transcription_server_port = os.getenv("SERVER_PORT", 8003)
        transcription_url = (
            f"http://{transcription_server_host}:{transcription_server_port}/transcribe"
        )

        # We need segments for the database
        params = {"include_segments": "true"}
        transcription_files = {
            "audio": (file.filename, file_content, file.content_type)
        }

        try:
            transcription_response = requests.post(
                transcription_url, params=params, files=transcription_files
            )
