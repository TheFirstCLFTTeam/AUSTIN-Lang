...
def test_transcribe_full_success(client, monkeypatch):
    ...
    with patch("transcription_orchestrator.requests.post") as mock_post:
        ...
        response = client.post("/transcribe/", files={"file": ("test.wav", io.BytesIO(b"content"), "audio/wav")})
...
def test_transcribe_submission_client_error(client):
    ...
    with patch("transcription_orchestrator.requests.post") as mock_post:
        ...
        response = client.post("/transcribe/", files={"file": ("test.txt", io.BytesIO(b"data"), "text/plain")})
...
def test_transcribe_non_json_error(client):
    ...
    mock_submission_resp.raise_for_status.side_effect = requests.exceptions.HTTPError("502 Server Error")

    with patch("transcription_orchestrator.requests.post") as mock_post:
        mock_post.return_value = mock_submission_resp
        response = client.post("/transcribe/", files={"file": ("test.wav", io.BytesIO(b"data"), "audio/wav")})
...
def test_transcribe_missing_segments(client, monkeypatch):
    ...
    m3 = MagicMock(status_code=200); m3.json.return_value = 1
    m4 = MagicMock(status_code=200); m4.json.return_value = 10
    m5 = MagicMock(status_code=200); m5.json.return_value = 100
...
def test_transcribe_raw_transcript_registration_failure(client, monkeypatch):
    ...
    m1 = MagicMock(status_code=200); m1.json.return_value = {"message": "ok"}
    m2 = MagicMock(status_code=200); m2.json.return_value = {"text": "ok", "segments": []}
    m3 = MagicMock(status_code=200); m3.json.return_value = 1 # audio_file_id success
