...
import { applyEdits, buildDiffView, computeWER, totalRawWords, rawWordIntervals, recomputeSegmentEdits } from "../../../../lib/transcriptEdits";
import { users, MOCK_CLIENTS, MOCK_USER_PROFILES } from "../../../../services/mock_data-users";
  ...
  return (
    <div className="mb-1">
      ...
      <span
        ...
        style={{ color, borderBottom: "1px dashed currentColor", paddingBottom: "1px" }}
  ...
  return (
    <>
      <button
        ...
        title={copied ? "Copied!" : (hasEdits ? "Copy edited transcript (right-click for options)" : "Copy transcript")}
      >
        {copied ? (
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#1a7f37" strokeWidth="2" strokeLinecap="square">
            <polyline points="20 6 9 17 4 12" />
          </svg>
        ) : (
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#7a7574" strokeWidth="1.75" strokeLinecap="square">
      ...
      {menuOpen && (
        ...
        >
          <button
            onClick={(e) => { e.stopPropagation(); copyText(editedText); }}
          ...
          <button
            onClick={(e) => { e.stopPropagation(); copyText(rawText); }}
            className="w-full px-4 py-2 text-left text-[0.75rem] cursor-pointer flex items-center gap-2"
            style={{ backgroundColor: "transparent", border: "none", color: "rgba(255,255,255,0.7)" }}
            onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = "rgba(255,255,255,0.1)"; }}
            onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = "transparent"; }}
          >
...
export default function FileDetailPage() {
  ...
  function logAction(type, description) {
    setActionHistory((h) => [{ type, description, timestamp: new Date() }, ...h]);
  }
  ...
  function applyMaskAcrossSegments(word) {
    ...
    for (const seg of rawSegments) {
      const applied = appliedSegments.find((s) => s.id === seg.id) || seg;
      const words = (applied.text || "").trim().split(/\s+/).filter(Boolean);
      const hasWord = words.some((w) => w === word);
      if (!hasWord) continue;
      const masked = words.map((w) => (w === word ? "[MASK]" : w)).join(" ");
      nextEdits = recomputeSegmentEdits(nextEdits, seg.id, seg.text, masked, { editedBy: user?.id });
      maskedCount++;
    }
    if (maskedCount > 0) {
      commitEdits(nextEdits, `Masked "${word}" across ${maskedCount} segment${maskedCount === 1 ? "" : "s"}`);
    }
  }
  ...
  useEffect(() => {
    function onKey(e) {
      ...
      if (e.key === "ArrowDown" && editedSegmentIds.length > 0) {
        e.preventDefault();
        jumpToEdit("next");
        return;
      }
  ...
  if (!fileData) return (
    <div className="flex items-center justify-center py-20">
      <div className="flex flex-col items-center gap-4">
        ...
        />
        <p className="text-[0.875rem] font-medium" style={{ color: "#7a7574" }}>Loading transcript...</p>
      </div>
    </div>
  );
  ...
  function getSpeakerSlot(seg, index) {
    return seg.speaker || (index % 2 === 0 ? "speaker_0" : "speaker_1");
  }
  ...
  function assignSpeaker(person) {
    if (alternateMode) {
      ...
      const targetSeg = rawSegments.find((s) => s.id === assignSegId);
      ...
      const count = rawSegments.filter((seg, i) => getSpeakerSlot(seg, i) === targetSlot).length;
      commitSpeakerMap(nextMap, `Assigned ${person.name} to ${count} segments (all ${targetSlot === "speaker_0" ? "Speaker 1" : "Speaker 2"})`);
    } else {
      const nextMap = { ...speakerMap, [assignSegId]: person };
      const segIdx = rawSegments.findIndex((s) => s.id === assignSegId);
      commitSpeakerMap(nextMap, `Assigned ${person.name} to segment ${segIdx + 1}`);
    }
    setAssignSegId(null);
    setSpeakerSearch("");
  }
  ...
  function unassignSpeaker(segId) {
    ...
    const segIdx = rawSegments.findIndex((s) => s.id === segId);
    commitSpeakerMap(nextMap, `Removed ${removed?.name || "speaker"} from segment ${segIdx + 1}`);
    setAssignSegId(null);
    setSpeakerSearch("");
  }
  ...
  return (
    <div>
      ...
      <div className="flex gap-6">
        <div className="flex-1">
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  style={{ backgroundColor: "#1c1b1b", border: "none", borderRadius: "0px", color: "#ffffff" }}
                >
                  <svg
              ...
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    style={{ accentColor: "#b20100", width: "80px" }}
                  />
                </div>
                <div className="flex items-center gap-2">
            ...
            >
              <div className="absolute inset-0 flex items-center justify-center gap-px px-2 pointer-events-none">
                {Array.from({ length: 80 }).map((_, i) => {
                  ...
                  return (<div key={i} className="w-1" style={{ height: `${h}px`, backgroundColor: i / 80 < progress ? "#b20100" : "#c4c4c4" }} />);
                })}
              </div>
            </div>

            <div className="text-center text-[0.75rem] font-mono" style={{ color: "#7a7574" }}>
          ...
          <div className="flex items-center justify-between px-4 py-2 mb-4" style={{ backgroundColor: "#ffffff", border: "1px solid #f0edec" }}>
            <div className="flex items-center gap-1">
              <ToolbarButton onClick={undo} disabled={undoStack.length === 0} title="Undo text edit (Ctrl+Z)">
              ...
              <ToolbarButton onClick={redo} disabled={redoStack.length === 0} title="Redo text edit (Ctrl+Shift+Z)">
              ...
              <ToolbarButton onClick={undoSpeaker} disabled={speakerUndoStack.length === 0} title="Undo speaker assignment">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></svg>
              </ToolbarButton>
              <ToolbarButton onClick={redoSpeaker} disabled={speakerRedoStack.length === 0} title="Redo speaker assignment">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /><path d="M16 3l2 2-2 2" /></svg>
              </ToolbarButton>
              <div className="w-px h-5 mx-1" style={{ backgroundColor: "#f0edec" }} />
              ...
              <div className="w-px h-5 mx-1" style={{ backgroundColor: "#f0edec" }} />
              <ToolbarButton onClick={() => jumpToEdit("prev")} disabled={editedSegmentIds.length === 0} title="Previous edit (↑)">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="18 15 12 9 6 15" /></svg>
              </ToolbarButton>
              <ToolbarButton onClick={() => jumpToEdit("next")} disabled={editedSegmentIds.length === 0} title="Next edit (↓)">
          ...
          <div className="space-y-6">
            {rawSegments.map((rawSeg, i) => {
              ...
              const speaker = getSpeakerName(rawSeg.id, i);
              const isAssigned = !!speakerMap[rawSeg.id];
              ...
              return (
                <div key={rawSeg.id} id={`seg-${rawSeg.id}`} className="flex gap-6 cursor-pointer" onClick={() => handleSeek(rawSeg.start)}>
                  <div className="w-24 shrink-0 pt-1">
                    <SpeakerLabel speaker={speaker.toUpperCase()} time={formatTime(rawSeg.start)} onAssign={() => setAssignSegId(rawSeg.id)} />
                    {isAssigned && (
                      <span className="text-[0.5625rem] block mt-0.5" style={{ color: "#004ec6" }}>
                        {speakerMap[rawSeg.id].type === "client" ? speakerMap[rawSeg.id].company : "UBS"}
                      </span>
                    )}
                    {editCount > 0 && (
                  ...
                  <div className="flex-1 p-4 transition-colors relative group/seg" style={{ backgroundColor: isActive ? "rgba(178, 1, 0, 0.06)" : "transparent" }}>
                    <CopySegmentButton rawText={rawSeg.text} editedText={appliedText} hasEdits={editCount > 0} />
                    {editingSeg === rawSeg.id ? (
                      <p
                        ...
                        onBlur={(e) => {
                          ...
                          updateText(rawSeg.id, newText);
                          setEditingSeg(null);
                        }}
                        onKeyDown={(e) => {
                          if (e.key === "Escape") { e.preventDefault(); setEditingSeg(null); return; }
                          // Alt+Space: replace a single highlighted word with [MASK]
                          if (e.altKey && e.key === " ") {
                            e.preventDefault();
                            const sel = window.getSelection();
                      ...
                      >
                        {tokens.map((tok, ti) => {
                          ...
                          const isMatch = q.length > 0 && tok.text.toLowerCase().includes(q);
                          const searchBg = "rgba(245, 183, 0, 0.45)"; // yellow, wins over audio highlight when both apply
                          const base = {
                            padding: "0 1px",
                            borderRadius: "2px",
                            ...
                            return (
                              <span
                                ...
                                style={{
                                  ...
                                  backgroundColor: isMatch ? searchBg : "transparent",
                                }}
                            ...
                            return (
                              <span
                                ...
                                style={{
                                  ...
                                  backgroundColor: isMatch ? searchBg : (isWordActive ? "rgba(178, 1, 0, 0.15)" : "transparent"),
                                  cursor: "pointer",
                                }}
                          ...
                          return (
                            <span
                              ...
                              style={{
                                ...
                                backgroundColor: isMatch ? searchBg : (isWordActive ? "rgba(178, 1, 0, 0.20)" : "transparent"),
        ...
        <div className="w-64 shrink-0 space-y-6">
          <div className="p-4" style={{ backgroundColor: "#ffffff" }}>
            ...
            <p className="text-[0.6875rem]" style={{ color: "#7a7574" }}>{edits.length} of {totalRawWords(rawSegments)} words corrected</p>
          </div>
      ...
      <Dialog open={assignSegId !== null} onClose={() => { setAssignSegId(null); setSpeakerSearch(""); }} title="Assign Speaker">
        <input
          value={speakerSearch}
        ...
        <label className="flex items-center gap-2 px-1 py-2 mb-3 cursor-pointer select-none" style={{ borderBottom: "1px solid #f0edec" }}>
          <input
            type="checkbox"
          ...
          />
          <div>
            <span className="text-[0.8125rem] font-medium" style={{ color: "#1c1b1b" }}>Apply to all matching speakers</span>
            <p className="text-[0.625rem]" style={{ color: "#7a7574" }}>Assigns this person to every segment from the same speaker slot</p>
          </div>
        </label>

        {assignSegId && speakerMap[assignSegId] && (
          <div className="flex items-center justify-between px-3 py-2 mb-3" style={{ backgroundColor: "rgba(0, 78, 198, 0.05)", border: "1px solid rgba(0, 78, 198, 0.15)" }}>
            <div className="flex items-center gap-2">
              <img src={speakerMap[assignSegId].profilePic || "/default_pfp.png"} alt="" className="w-5 h-5 object-cover" style={{ borderRadius: "0px" }} />
              <span className="text-[0.75rem]" style={{ color: "#004ec6" }}>
                Assigned: <strong>{speakerMap[assignSegId].name}</strong>
              </span>
            </div>
            <button
              onClick={() => unassignSpeaker(assignSegId)}
        ...
        <div style={{ maxHeight: "280px", overflowY: "auto" }}>
          <SpeakerPickerSection
            label="Operators (UBS)"
            items={users.map((u) => ({
              id: u.id, name: MOCK_USER_PROFILES[u.id]?.name || u.name, company: "UBS", role: u.role, type: "operator",
              profilePic: MOCK_USER_PROFILES[u.id]?.profilePic || "/default_pfp.png",
            }))}
            search={speakerSearch}
            onPick={assignSpeaker}
            currentId={assignSegId ? speakerMap[assignSegId]?.id : null}
          />
          <SpeakerPickerSection
            label="Clients"
            items={MOCK_CLIENTS.map((c) => ({
              id: c.id, name: c.name, company: c.company, role: c.relationship, type: "client",
      ...
      <Dialog open={pendingMask !== null} onClose={() => setPendingMask(null)} title="Apply Mask Globally?">
        <p className="text-[0.8125rem] mb-2" style={{ color: "#1c1b1b" }}>
          You replaced <strong style={{ fontFamily: "monospace", backgroundColor: "#f6f3f2", padding: "1px 4px" }}>{pendingMask?.word}</strong> with <strong style={{ fontFamily: "monospace", backgroundColor: "rgba(178, 1, 0, 0.08)", color: "#b20100", padding: "1px 4px" }}>[MASK]</strong> in this segment.
        </p>
        <p className="text-[0.8125rem] mb-5" style={{ color: "#7a7574" }}>
          Would you like to mask every occurrence of this word across all segments in this transcript?
        </p>
        <div className="flex items-center justify-end gap-3">
          ...
          <button
            onClick={() => {
              if (pendingMask) applyMaskAcrossSegments(pendingMask.word);
              setPendingMask(null);
            }}
...
function VolumeIcon({ muted, level }) {
  ...
  return (
    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5" />
      {level > 0.05 && <path d="M15.54 8.46a5 5 0 0 1 0 7.07" />}
      {level > 0.55 && <path d="M19.07 4.93a10 10 0 0 1 0 14.14" />}
    </svg>
  );
}
...
function ToolbarButton({ children, onClick, disabled, title, active }) {
  return (
    <button
      ...
      style={{
        backgroundColor: active ? "rgba(178, 1, 0, 0.08)" : "transparent",
        border: "none",
        borderRadius: "2px",
        color: active ? "#b20100" : "#1c1b1b",
      }}
      onMouseEnter={(e) => { if (!disabled && !active) e.currentTarget.style.backgroundColor = "#f6f3f2"; }}
...
function SaveStatus({ status, isDirty, actionHistory, showHistory, onToggleHistory, historyRef, onUndoText, onUndoSpeaker, canUndoText, canUndoSpeaker }) {
  let label;
  let color;
  if (status === "saving") { label = "Saving…"; color = "#7a7574"; }
  else if (status === "error") { label = "Save failed"; color = "#b20100"; }
  else if (isDirty) { label = "Unsaved"; color = "#7a7574"; }
  else { label = "Saved"; color = "#1a7f37"; }
  ...
  const iconForType = (type) => {
    if (type === "text") return (
      <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#7a7574" strokeWidth="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" /><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" /></svg>
    );
    if (type === "speaker") return (
      <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#7a7574" strokeWidth="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></svg>
    );
    return (
      <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#7a7574" strokeWidth="2"><path d="M3 7v6h6" /><path d="M21 17a9 9 0 0 0-9-9 9 9 0 0 0-7 3L3 13" /></svg>
    );
  };
  ...
  return (
    <div className="relative" ref={historyRef}>
      <button
        ...
        style={{ color, backgroundColor: "transparent", border: "none" }}
        title="View action history"
      >
        {label}
        {actionHistory.length > 0 && (
          <span className="text-[0.5625rem] font-bold" style={{ color: "#7a7574" }}>({actionHistory.length})</span>
        )}
        <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" style={{ transform: showHistory ? "rotate(180deg)" : "rotate(0)", transition: "transform 150ms" }}>
      ...
      {showHistory && (
        ...
        >
          <div className="flex items-center justify-between px-3 py-2" style={{ borderBottom: "1px solid #f0edec" }}>
            <span className="text-[0.6875rem] font-bold uppercase tracking-wider" style={{ color: "#1c1b1b" }}>Action History</span>
            <div className="flex items-center gap-1">
              <button
                ...
                style={{ backgroundColor: "transparent", border: "1px solid #f0edec", color: "#1c1b1b" }}
          ...
          <div style={{ maxHeight: "240px", overflowY: "auto" }}>
            {actionHistory.length === 0 ? (
              <p className="px-3 py-4 text-center text-[0.75rem]" style={{ color: "#7a7574" }}>No actions yet</p>
            ) : (
              actionHistory.map((action, i) => (
                <div key={i} className="flex items-start gap-2 px-3 py-2" style={{ borderBottom: "1px solid #f6f3f2" }}>
                  <div className="mt-0.5 shrink-0">{iconForType(action.type)}</div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[0.75rem] truncate" style={{ color: "#1c1b1b" }}>{action.description}</p>
                    <p className="text-[0.625rem]" style={{ color: "#7a7574" }}>{formatTimestamp(action.timestamp)}</p>
                  </div>
                </div>
              ))
            )}
...
function HelpOverlay({ onClose }) {
  const rows = [
    ...
    ["Mask a word",               "Select word + Alt+Space"],
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-[0.875rem] font-bold uppercase tracking-wider" style={{ color: "#1c1b1b" }}>Keyboard Shortcuts</h3>
          <button onClick={onClose} className="text-[0.75rem] cursor-pointer" style={{ backgroundColor: "transparent", border: "none", color: "#7a7574" }}>Esc</button>
        </div>
        <div className="space-y-2">
          {rows.map(([label, key]) => (
            <div key={label} className="flex items-center justify-between py-1">
              <span className="text-[0.8125rem]" style={{ color: "#1c1b1b" }}>{label}</span>
              <span className="text-[0.6875rem] px-2 py-0.5 font-mono" style={{ backgroundColor: "#f6f3f2", color: "#1c1b1b", borderRadius: "2px" }}>{key}</span>
            </div>
          ))}
        </div>
      </div>
...
function SpeakerPickerSection({ label, items, search, onPick, currentId }) {
  const q = search.trim().toLowerCase();
  const filtered = q
    ? items.filter((p) => p.name.toLowerCase().includes(q) || p.company.toLowerCase().includes(q))
    : items;
  if (filtered.length === 0) return null;
  return (
    <div className="mb-3">
      <p className="text-[0.625rem] font-semibold uppercase tracking-wider mb-1 px-1" style={{ color: "#7a7574" }}>{label}</p>
        ...
        {filtered.map((person) => {
          ...
          return (
            ...
            >
              ...
              />
              <div className="flex-1 min-w-0">
                <p className="text-[0.8125rem] font-medium truncate">{person.name}</p>
                <p className="text-[0.6875rem] truncate" style={{ color: "#7a7574" }}>{person.company} &middot; {person.role}</p>
              </div>
              {isSelected && (
