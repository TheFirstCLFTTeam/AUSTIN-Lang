...
| Leaderboard | 🟡 endpoint + FE wiring shipped (real submissions ranked once `training_job_id` + `submitted_by` are stamped on `model_evaluation`); `eng-priya`/`eng-andreas`/`eng-james` mock-id reconciliation with `users.db` still pending | FE + BE | this doc §3.2 |
| Retraining / Training Jobs | 🟡 in progress (orchestrator + simulated + real CLI-driven worker + heartbeat/log SSE + script upload + list-page real-data swap + base-model registry slice A + fingerprint algorithm + leaderboard endpoint shipped; artifact upload pending) | FE + BE | this doc §3.3, [`training-job-pipeline.md`](training-job-pipeline.md) |
...
| Financial terms dictionary | 🟡 slices 1-5 shipped (microservice + 10 endpoints + 47 BE tests + CSV cleaner 6,013/303 split + FE proxies + admin moderation page + "💼 Add to dictionary" affordance + auto-trail occurrence hook + `financial_term_accuracy` strategy + dataset-builder snapshot helpers + 22 FE tests); eval-manifest writer that calls `stamp_critical_terms` is the last open thread | new BE service + FE admin page | [`financial-terms-dictionary.md`](financial-terms-dictionary.md) |
...
1. ✅ Implementations for the remaining dashboard metrics — **9 of 9 done 2026-04-28**. WER, CER, SMR, F1 shipped earlier; this round added **`punctuation_accuracy`** (no manifest metadata needed — F1 over punctuation positions via Wagner-Fischer trace-back), **`weighted_wer`** (consumes new `Sample.info_values`, falls back to uniform weights when the manifest doesn't supply them, caps per-sample at 1.0), **`language_cer`** (groups by new `Sample.language` BCP-47 tag, macro-averages across languages with per-language breakdown), **`entity_f1`** (closed-set span recovery from new `Sample.entity_spans` half-open char ranges; per-label breakdown surfaces `f1.PERSON` / `f1.ORG` / etc. for ranking miss patterns), **`english_accuracy`** (per-language WER → 1-WER accuracy, accepts `en` + regional variants `en-us` / `en-gb` / `en-sg` / etc.), **`mandarin_accuracy`** (sibling for `zh` / `zh-cn` / `zh-tw` / `cmn`; Cantonese explicitly excluded for the future `cantonese_accuracy`; surfaces `tokenisation_warning` in breakdown when avg-tokens-per-sample < 3, signalling unsegmented CJK that would benefit from `jieba`). `Sample` dataclass + `manifest_loader.py` extended to carry + validate the three new fields with type + range checks; legacy manifests stay valid via `None` defaults. New shared `_per_language.py` helper makes adding a new per-language strategy a 3-line file. The merge layer (`live-dashboard-metrics.js::formatMetricValue` + `formatSublabel`) gained a fraction → percentage scaling step so strategy values in [0, 1] display correctly in cards with `unit: "%"` — applies retroactively to all six new strategies. **42 new pytest cases this round** (6 punctuation + 6 weighted_wer + 4 language_cer + 5 entity_f1 + 8 manifest_loader + 7 english_accuracy + 6 mandarin_accuracy + helper-direct cases). All 183 metrics-service tests green at the 6-of-9 mark.

   **Closing the last 3 (round 3, same day):** `Sample` extended with `speakers` (ref-side char-span speaker labels), `hypothesis_speakers` (hyp-side, will be populated when a diarisation post-processor lands), and `language_spans` (intra-sentence language tags as half-open char ranges). `manifest_loader.py` factored the validation into a `_parse_span_list()` helper so `entity_spans`, `speakers`, `hypothesis_speakers`, and `language_spans` all share one validator (rejects malformed shapes / negative or inverted ranges). New `code_switch_pier` (closes CS-PIER card — measures fraction of language-tagged spans the model dropped, per-language `recovered.<lang>` + `total.<lang>` breakdown), `word_diarization_error` (char-aligned ref ↔ hyp speaker-label disagreement, skip-on-no-hypothesis-speakers — every real sample lands in the skip bucket today since the ASR pipeline doesn't produce hyp-side speakers; auto-flows when WhisperX/pyannote lands), `speaker_diarization` (Jaccard distance over the speaker sets — proxy for time-aligned DER, same skip-on-no-hyp-speakers semantics). Shared `_diarization.py` helper handles char→speaker assignment + skip counting. **19 new pytest cases** (5 code_switch_pier + 4 WDER + 4 speaker_diarization + 6 manifest_loader). All 202 metrics-service tests green.

   ✅ **Sub-item 3 — `METRICS_BASELINE_FRESHNESS_DAYS` enforcement** — done 2026-04-28 (was a separate pending bullet below). New `MetricsStore.latest_baseline_age_days(base_model, dataset_name)` + `is_baseline_fresh(...)` helpers query the latest base-only row and return age in days (None when missing). `POST /evaluations/baseline` short-circuits with a `BaselineFreshnessResponse` ({skipped: true, reason, evaluation_id, age_days}) when the latest row is within the freshness window (default 30 days, env-overridable). New `force=true` flag bypasses the short-circuit. Post-train hook (`backend/retraining-pipeline/post_train_hook.py`) gained a `refresh_baseline=True` default that fires `POST /evaluations/baseline` before the adapter run; the metrics-service decides whether to skip or actually run. Fail-soft: a flaky baseline call doesn't block the adapter run. `_maybe_refresh_baseline` helper extracted for testability. 13 new metrics-service pytest cases for the storage + endpoint behaviour + 4 retraining-pipeline cases for the hook integration (baseline-then-run order, refresh_baseline=False skips the call entirely, baseline failure isolation, skipped-response logged at INFO). All 31 retraining-pipeline tests + 183 metrics-service tests green; 10 existing FE merge-layer tests still pass with the percentage-scaling change.
2. ~~Inference fan-out in `/evaluations/run` — today the eval manifest must carry `hypothesis`. A follow-up PR will let it be omitted and have metrics-service POST each clip to `transcription-service-2:/transcribe?domain=<adapter>` to fill it in.~~ _Done 2026-04-28._ New `backend/metrics_service/inference_client.py` runs a `ThreadPoolExecutor`-backed fan-out (default 4 workers, env-overridable via `INFERENCE_FANOUT_CONCURRENCY` — bounded so a 1000-clip manifest doesn't deepen TS-2's GPU queue past usability). Per-clip failures (network timeout, missing audio file, non-2xx HTTP) leave the slot `None` instead of raising — the existing strategies already score `None` hypothesis as a fully-wrong sample. `Sample.hypothesis` widened from `str` to `Optional[str]` so the manifest can distinguish three cases: `"hello"` (model produced text), `""` (model deliberately produced silence — no fan-out), `None` (manifest omitted, fan out). `manifest_loader.py` reads the JSONL with that distinction. `_fill_in_hypotheses` helper in `main.py` is called from both `/evaluations/run` (passes `domain=<adapter>`) and `/evaluations/baseline` (passes `domain=base`); the fan-out counts (`fanout: N clips inferred, M failed`) are appended to the evaluation `notes` so a reviewer sees them on `GET /evaluations/{id}`. Manifests that omit `hypothesis` AND `audio_path` get a 400 with helpful detail rather than a silent empty sample. Compose adds `TRANSCRIPTION_SERVICE_2_URL` (defaults to `http://transcription-service-2:8005`) and `INFERENCE_FANOUT_CONCURRENCY` env vars; metrics-service `depends_on` extended. **17 new pytest cases** (3 manifest_loader None-vs-empty, 2 split helper + 8 fan-out client + 5 endpoint integration) — total 219 metrics-service tests green.
3. ~~`METRICS_BASELINE_FRESHNESS_DAYS` enforcement — the schema and pair query support it; the post-train hook doesn't yet check it before triggering a baseline re-run. Default 30 days.~~ _Done 2026-04-28 — see sub-item under #1 above._
...
**Endpoint + FE wiring shipped this branch (2026-04-28).** New `GET /leaderboard` on `metrics-service` ranks real evaluations once the post-train hook stamps `training_job_id` + `submitted_by` on the row; FE proxy `/api/leaderboard` + service helper `fetchLeaderboard()` + leaderboard page now consume the live list with mock fallback.

What landed:

- **Schema:** `model_evaluation` gains nullable `training_job_id` + `submitted_by` columns via the same idempotent ALTER pattern used for `script_*` on the orchestrator (so a long-running `metrics_data` volume picks them up at next boot, no manual migration). Existing seeded mock rows + ad-hoc evals stay valid; the leaderboard query just won't carry an engineer attribution for those.
- **Storage:** `MetricsStore.list_leaderboard(dataset_name, base_family?, only_finetuned=True, limit=100)` — picks the latest evaluation per `(base_model, COALESCE(adapter_name, '__base__'))` pair via correlated subquery (portable across SQLite versions instead of using window functions), pulls WER/CER/RTF in a single `IN (…)` query, pivots them onto `LeaderboardRow`, sorts by WER ascending with NULL-WER rows sinking. `base_family` is a case-insensitive substring filter on `base_model` so it works for `openai/whisper-…` HF ids and bare-id rows alike.
- **HTTP:** `GET /leaderboard?dataset_id=…&base_family=…&only_finetuned=…&limit=…` returns `{dataset_id, base_family, rows: [{rank, base_model, base_family, adapter_name, adapter_version, model_name, submitted_by, training_job_id, evaluation_id, evaluated_at, wer, cer, rtf}]}`. Rank is server-stamped 1-based. `base_family` derived per row by stripping vendor prefix and taking the family head (`openai/whisper-large-v3-turbo` → `whisper`); replaced by registry lookup once the base-model registry on the orchestrator becomes the canonical source. Empty rows on unknown dataset is a 200 with `{rows: []}`, not a 404 — distinguishes "no real submissions yet" from "upstream down" so the FE can pick the right fallback banner.
- **FE proxy:** `frontend/src/app/api/leaderboard/route.js` — requireUser-gated, 15 s timeout, distinguishes 404 (dataset unknown) from 502 (upstream down). 400 when `dataset_id` missing.
- **FE service:** `services/metrics.js::fetchLeaderboard({datasetId, baseFamily?, onlyFinetuned?, limit?})`.
- **FE wiring:** `leaderboard/page.jsx` adds a `useEffect` keyed on the dataset selection that fetches live rows + adapts them via `adaptLiveLeaderboardRow` (engineer attribution looked up against `mock_data-users` for now — falls through to the user id when the row stamps `submitted_by` with a non-mock id). `liveSource` state drives a small badge: green `LIVE` when real rows arrive, blue `DEMO DATA — NO LIVE SUBMISSIONS YET` when real mode + zero rows fall back to mock, deep-red `METRICS UNREACHABLE — SHOWING MOCKS` on fetch failure. `rows` + `familyOptions` memos prefer live rows when available; mock mode (`NEXT_PUBLIC_MOCK_API=true`) bypasses the fetch entirely so dev keeps working with the existing fixture.
- **Tests:** 14 new pytest cases in `metrics_service/tests/test_leaderboard.py` (storage: empty dataset, ranking, latest-per-pair, only-finetuned default + override, base-family filter, NULL-WER row sinks; endpoint: empty 200, rank stamping, metric pivot, base-family filter, model-name string includes adapter, family extracted from HF id, missing-param 422). All 128 metrics-service tests green.

**Pending follow-ups:**

1. **Engineer-id reconciliation.** Mock leaderboard rows use `eng-priya`, `eng-andreas`, `eng-james` — none of these exist in `users.db`. The live endpoint already returns whatever `submitted_by` was stamped at eval-recording time (real engineer ids), so once real submissions accumulate the page surface is correct; the mock fixtures need a one-shot pass to remap to real `users.db` ids before the demo loop runs against a hybrid of mock + real rows.
2. **Server-side group scoping.** Today the FE applies `getGroupsForUser()` after fetching all rows — that's defence-in-depth but the rows leak across groups before scoping. Move the `group_id` filter to the SQL `WHERE` once `group_id` is stamped on `model_evaluation` (or joined from `users.db` over a cross-service link).
3. **Aggregated `submissionCount`.** The endpoint returns one row per `(base_model, adapter_name)` pair — the page's `submissionCount` field is hard-coded to 1 in the live adapter. Aggregate at SQL level (count per submitter, or per adapter family) when the demo needs that signal back.
...
2. ~~**Heartbeat + log SSE** for the detail page — replaces `MOCK_COMPLETION_DELAY_MS = 8000` and `getTrainingLogs()`.~~ _Done 2026-04-28._ New `backend/training_orchestrator/sse.py` exposes a generator that replays the existing log file → tails new lines → polls `JobStore` for state changes → closes on terminal state. Three named SSE events (`log` / `state` / `end`) plus periodic heartbeat comments to keep proxies warm. Wired as `GET /jobs/{id}/logs` (StreamingResponse, `text/event-stream`). FE proxy at `frontend/src/app/api/training-jobs/[id]/logs/route.js` pass-through-streams via `requireUser`-gated ReadableStream. New `<TrainingJobProgress>` component (status badge + animated progress bar + auto-scrolling terminal-style log panel + 5,000-line cap to avoid browser OOM) consumes the EventSource on `/training/[id]`. Real mode replaces the static `<LogPanel>` + the `MOCK_COMPLETION_DELAY_MS` bandaid; mock mode keeps the static panel so dev still has something to look at. 7 new pytest cases in `tests/test_sse.py` (event-shape format, ghost-job error path, terminal-at-start short-circuit, log replay + end ordering, state transitions during stream, failure propagation with `failure_reason`, heartbeat fires when idle). All 61 orchestrator tests green (54 prior + 7 new).
3. ~~**Script upload** (`POST /jobs/{id}/script`) — multipart, size-capped, MIME-sniffed (same harness as F6).~~ _Done 2026-04-28._ New `backend/training_orchestrator/script_storage.py` mirrors the F6 audio-upload harness for Python source: 256 KB cap (`MAX_SCRIPT_BYTES`), declared-MIME allow-list (`text/x-python`, `text/plain`, `application/octet-stream`, …), `.py` extension gate, magic-byte sniff that rejects null-byte / non-UTF-8 / non-Python content, SHA-256 stamp, atomic temp-file + rename to `TRAINING_SCRIPT_DIR/{job_id}/script.py`. `JobStore.attach_script` only writes while the job is `queued` (frozen once a worker claims it — 409). Re-uploads replace the prior file in-place. `schema.sql` adds four columns (`script_filename`, `script_sha256`, `script_size_bytes`, `script_uploaded_at`) with idempotent `ALTER TABLE` retro-fit so an existing `training_orchestrator.db` volume gets the columns at next boot. New endpoint `POST /jobs/{id}/script` (FastAPI `UploadFile`); FE proxy at `frontend/src/app/api/training-jobs/[id]/script/route.js` (requireUser-gated multipart pass-through, 30 s timeout, buffers via `arrayBuffer()` so the upstream parser sees the original boundary). Dialog at `training/page.jsx` got an optional `<input type="file" accept=".py">` (cloud + federated targets only — local target's bash entrypoint stays a path string), hooked to `uploadTrainingScript()` after `submitTrainingJob()` resolves; upload failure surfaces on the dialog with the orchestrator's verbatim `detail` and does not roll the job back (engineer can retry against the queued job). 27 new pytest cases on top of the existing 61 (12 pure-function: sniffer accepts canonical script + module-docstring head, rejects binary / empty / invalid-UTF-8 / oversize / wrong-MIME / wrong-extension / null-byte-with-py-extension / empty-body; traversal-stripped filename round-trip; replace-prior overwrites on disk; default cap matches doc; 4 storage: attach happy path + 404 missing + 409 after preparing + idempotent-re-attach-while-queued; 11 endpoint: happy path + 413 oversize + 415 wrong-MIME + 415 wrong-extension + 415 binary + 400 empty + 404 missing-job + 409 after-running + replace-prior round-trip on disk + detail read carries the new columns). All 88 orchestrator tests green.
4. 🟡 **`base_model` + `training_artifact` registry tables** + **architecture-deviation check** — slice A done 2026-04-28; slice B (artifact upload + orchestrator-side fingerprint wiring) pending.

   **Slice A landed (registry tables, vendor seed, read-only HTTP, fingerprint algorithm).** New tables on the orchestrator's SQLite — `base_model` (id, family, display_name, hf_id, owner_user_id, parent_base_model_id, architecture_fingerprint, weights_uri, data_zone, created_at) and `training_artifact` (id, job_id, kind ∈ {adapter, base_model, checkpoint, candidate_base}, base_model_id FK, uri, sha256, size_bytes, arch_fp, weight_fp, adapter_fp, promotion_note, created_at), plus a nullable `base_model_id` FK column on `training_job` (the existing `base_model` HF-id string column stays canonical for backwards compat). Idempotent ALTER on the existing `training_job` table mirrors the script-upload retro-fit so a long-running DB volume picks the new column up at next boot. New `backend/training_orchestrator/registry.py` exposes `BaseModelStore` (seed_vendors / get / find_by_hf_id / list with family + owner filters / register_derived with parent-FK validation) and `ArtifactStore` (record / get / list_for_job). Vendor seed list mirrors `BASE_MODELS` in `training/page.jsx` exactly (Whisper Large-v3, Whisper Tiny, MERaLiON, Qwen3-ASR) and runs from a FastAPI startup hook on every boot — idempotent by id, so a missing entry on a long-running DB back-fills automatically. New `backend/training_orchestrator/fingerprint.py` implements the §4.3 two-layer algorithm (`arch_fp` over the config allow-list, `weight_fp` over normalised parameter triples, optional `adapter_fp`) as a pure torch-free function — caller passes pre-extracted `(name, shape, dtype_str)` triples so the orchestrator container doesn't need to pull torch + transformers. `classify_promotion()` exposes the four-row promotion matrix (duplicate / adapter_or_finetune / candidate_base / new_base). New endpoints `GET /base-models` (with `family` / `owner_user_id` / `include_vendor` filters), `GET /base-models/{id}`, `GET /jobs/{id}/artifacts` — read-only for now since the artifact-upload contract belongs to slice B. 36 new pytest cases (12 store: seed idempotency + back-fill + filters + register_derived FK check + owner-list with/without vendor + artifact CRUD + invalid kind; 13 algorithm: determinism, runtime-key exclusion, real-arch diff detection, dtype collapse for float precision, dtype isolation for quantization, param-order independence, LoRA → adapter, LayerNorm modules_to_save → still adapter, lm_head → candidate_base, target-module-order independence, classify_promotion matrix × 4 rows, allow-list immutability; 6 endpoint: seeded vendor list + family filter + 404 + detail + artifacts-for-unknown-job 404 + artifacts-for-known-job empty list + 6 unspecified misc). All 124 orchestrator tests green (88 prior + 36 new).

   **Slice B pending (artifact upload + orchestrator-side fingerprint wiring).** `POST /jobs/{id}/artifacts` (multipart upload of `.safetensors` / `adapter_model.safetensors` + `adapter_config.json`) needs torch + transformers + safetensors in the container to open the file and extract `named_parameters()`. Two paths forward — (a) bake torch into the orchestrator image (~3 GB, slow CI), or (b) split a sidecar `fingerprint-service` container that does the read-and-extract step and returns triples to the orchestrator over HTTP, keeping `fingerprint.py` in-process. Once that's wired, the endpoint runs `fingerprint()` → `classify_promotion()` → on `outcome='new_base'` calls `BaseModelStore.register_derived` and writes a `training_artifact` row with `kind='base_model'` referencing the new id; on `outcome='candidate_base'` writes `kind='candidate_base'` and surfaces a human-review badge on `/training/[id]`; on `outcome='duplicate'` returns 409. Trust boundary stays the same — fingerprint runs server-side; the worker just uploads the file.
5. **Federated path** — schema carries `fl_enabled` + `target='federated'`; runtime returns 501 until F25 (FL framework ADR) lands.
6. ~~**Leaderboard endpoint** on metrics-service joining `model_evaluation` ⇄ `training_artifact` ⇄ `training_job` ⇄ `user` ([`training-job-pipeline.md`](training-job-pipeline.md) §4.7) — depends on (4).~~ _Done 2026-04-28._ Lives on `metrics-service` (closer to evaluation data than to job state, per §4.7). Took a denormalised path instead of a literal cross-service four-way join — `model_evaluation` gained nullable `training_job_id` + `submitted_by` columns stamped at eval-recording time, so the leaderboard query is a single-table read on `metrics.db` rather than HTTP fan-out to the orchestrator. Cross-service link is provided by the FK-by-string (the orchestrator's `training_job.id` literal stored in metrics.db) — the FE proxy can call `GET /api/training-jobs/{id}` for any row that needs richer context. Slice A's `training_artifact` table will plumb in once slice B (artifact upload) starts writing rows; the leaderboard endpoint already accepts the column shape it'll need (`evaluation_id` round-trips to find the producing artifact). Detail for the endpoint shape, FE wiring, and the mock-id reconciliation thread is in §3.2.
7. ~~**List page real-data swap.**~~ _Done 2026-04-28._ `frontend/src/services/training-jobs.js` gained `adaptOrchestratorJob(JobResponse)` — a pure projection from the orchestrator's response shape onto the row shape `/training/page.jsx` already consumes (status mapping `preparing→queued`, `evaluating→running`, `published→completed`, `cancelled`/`failed` kept as their own; LoRA/LR/epochs parsed from `env_json`; HF id reverse-mapped to display label via the existing `BASE_MODEL_HF_IDS` table; `started_at || submitted_at` formatted as `MMM DD, HH:MM GMT`; `progress_pct` rounded; gpu/metrics/lossHistory/infra zeroed pending the heartbeat hook). The page now polls `listTrainingJobs({ limit: 100 })` every 5 s in real mode (single in-flight guard via cancelled flag) + refreshes immediately after a successful submission so a freshly-launched job appears without a poll wait. Two fall-back banners cover the empty-but-no-error case ("DEMO DATA — NO LIVE JOBS YET" — shows the bundled `TRAINING_JOBS` fixture so the demo loop has something to render before any submission) and the upstream-error case ("ORCHESTRATOR UNREACHABLE — SHOWING MOCKS"). Mock mode (`NEXT_PUBLIC_MOCK_API=true`) bypasses polling and stays on the static fixture. `StatusBadge` extended with `completed` (green), `failed` (deep red), `cancelled` (grey) so terminal-state rows render correctly. 17 new vitest cases for the adapter (status projections, env parsing, LoRA off, HF reverse-map + family-tail fallback, time formatting, missing env, progress rounding, null progress, live flag, placeholder shape) — all 72 FE tests green.
...
**Slices 1-5 shipped this branch (2026-04-28).** `backend/Financial_terms_dictionary/` FastAPI microservice on port **8009** is up: schema + storage helpers + 10 endpoints + role gates + container + compose wiring + CSV bulk-import with cleaning heuristics + FE proxies + admin moderation page + selection-based "💼 Add to dictionary" affordance on the file detail page + auto-trail occurrence hook in `writeEditsForFile` + the `financial_term_accuracy` strategy in `metrics_service/strategies/` + the snapshot/stamp helpers in `retraining-pipeline/eval_manifest_helpers.py` that the future eval-manifest writer will call. The eval-manifest writer itself (the consumer of those helpers — replaces today's manual JSONL hand-curation) is the last open thread; see [`financial-terms-dictionary.md`](financial-terms-dictionary.md) §10.
...
| `METRICS_BASELINE_FRESHNESS_DAYS` | how long a baseline row is reused before re-running (default 30; consumed by `/evaluations/baseline` short-circuit + the post-train hook's `refresh_baseline=True` default) | ✅ wired |
| `TRANSCRIPTION_SERVICE_2_URL` | metrics-service → transcription-service-2 fan-out target for `/evaluations/run` (default `http://transcription-service-2:8005`) | ✅ wired |
| `INFERENCE_FANOUT_CONCURRENCY` | thread-pool size for the inference fan-out (default 4; bump for multi-GPU setups) | ✅ wired |
...
| `TRAINING_SCRIPT_DIR` | per-job directory for uploaded Python training scripts (default `/app/data/scripts`; per `POST /jobs/{id}/script`) | ✅ wired |
| `MAX_SCRIPT_BYTES` | cap for `POST /jobs/{id}/script` payloads (default 262144 = 256 KB) | ✅ wired |
