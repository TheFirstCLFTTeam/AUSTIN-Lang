...
**Update (2026-04-28):** orchestrator skeleton + simulated worker landed — `backend/training_orchestrator/` on port **8008** (the doc's earlier "port 8007" suggestion is stale; 8007 is taken by `meeting-webhooks`). What's live: `POST /jobs`, `GET /jobs`, `GET /jobs/{id}`, `POST /jobs/{id}/cancel`, `POST /jobs/{id}/transitions/{new_status}`, full state machine in `storage.py::JobStore`, FE proxy + `submitTrainingJob` client + LAUNCH-button wiring, **plus a `worker.py` simulated worker** that drives `queued → preparing → running → evaluating → published` in ~30s so the loop closes end-to-end without dragging torch into the orchestrator. The simulated worker is honest about what it is (module docstring + log lines call it out) and is gated by `WORKER_ENABLED` so a real worker can take over the queue without code changes. What's still missing: real-training worker (open question 1 below), heartbeat/log SSE, the `base_model` + `training_artifact` registry, and the leaderboard endpoint. See [`backend_integration_status.md`](backend_integration_status.md) §3.3 for the per-layer breakdown.
